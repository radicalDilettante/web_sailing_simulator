...
const obstacleAABBList: iAABB[] = [];
const goalAABB: iAABB = { center: vec2.create(), length: vec2.create() };
...
if (!program) alert("Cannot load shaders");
...
function animate(now: number) {
  ...
  mat4.multiply(
    mastModel,
    model,
    mast.getModelMatrix(yacht.getAppWindAngle(worldWind))
  );
  ...
  mat4.multiply(
    vaneModel,
    vaneModel,
    vane.getModelMatrix(yacht.getCurPos(), worldWind)
  );
...
function setObstacles() {
  for (let i = 0; i < NUM_OF_OBSTACLES; i++) {
    const center = vec2.fromValues(
      getRandomNum(0, 55) * getSign(),
      getRandomNum(0, 35) * getSign()
    );
    const length = vec2.fromValues(getRandomNum(2, 10), getRandomNum(2, 10));

    const obstacle = new Obstacle(gl);
    const aabb: iAABB = { center: vec2.create(), length: vec2.create() };
...
function testCollision() {
  for (let i = 0; i < NUM_OF_OBSTACLES; i++) {
    if (
      yacht.testCollision(
        obstacleAABBList[i].center,
        obstacleAABBList[i].length
      )
    ) {
      console.log("crash!!");
